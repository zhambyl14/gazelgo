...
class NameGuard {
  ...
  static String? validate(String raw) {
    final v = raw.trim();
    if (v.length < 4) return t('Атыңызды толық жазыңыз');
    if (!_lettersOnly.hasMatch(v)) {
      return t('Атта тек әріптер болуы керек (цифр/белгі жарамсыз)');
    }
    final words =
        v.split(RegExp(r'[\s\-]+')).where((w) => w.isNotEmpty).toList();
    if (words.length < 2 || words.any((w) => w.length < 2)) {
      return t('Аты-жөніңізді толық жазыңыз (мыс: Асхат Серіков)');
    }
    if (words.any((w) => _repeatedChar.hasMatch(w))) {
      return t('Атыңызды дұрыс жазыңыз');
    }
    final lower = v.toLowerCase();
    for (final bad in _profanity) {
      if (lower.contains(bad)) return t('Аты-жөнде рұқсат етілмеген сөз бар');
    }
    return null;
  }
}
