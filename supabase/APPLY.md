   ...
   - `migrations/0066_news_stories.sql` ← **ЖАҢАЛЫҚТАР (сторис)** (0065-тен
     ...
     **ӨШІРУЛІ** — Модератор → Баптаулар → «Жаңалықтар (сторис)» қосқышын
     ...
     · Жаңа `news_stories` кестесі: әр стористің медиасы, әуені, «Толығырақ»
       ...
       (`bg_index`, 0..5), жазу мен суреттің ЭКРАНДАҒЫ ОРНЫ (`layout` jsonb —
       Instagram-дағыдай еркін сүйреу/масштабтау + жазудың түсі),
       жарияланым терезесі (`starts_at` / `expires_at`) және АУДИТОРИЯСЫ
       (`audiences` — `client`, `executor`, `guest` немесе олардың кез
       келген комбинациясы).
     · **Файлды БҰРЫН орындап қойған болсаңыз — ҚАЙТА орындаңыз.**
       `bg_index` мен `layout` бағандары кейін қосылды: миграция оларды
       `alter table … add column if not exists` арқылы үстейді, ал
       `mod_news_save`-тің ЕСКІ қолтаңбалары әдейі `drop`-талады — әйтпесе
       базада бірнеше нұсқа қатар тұрып, қосымша «function ... is not
       unique» қатесін алар еді.
