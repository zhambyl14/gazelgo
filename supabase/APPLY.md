...
1. https://supabase.com/dashboard/project/xibxaqcrdpgyzohfplda/sql/new ашып,
   ...
   **0013, содан соң 0014, 0016, 0017, 0018, 0019, 0020, 0021, 0022, 0023,
   ...
   0036, … 0042, 0043, 0044, 0045, 0046, 0047, 0048, 0049, 0050, ең соңынан
   0051-ді орындаңыз.**
...
## Қолдау чатының AI кеңесшісі (support-assistant)

Модератордың қолдау чат экранында 🤖 батырмасы бар. Ол чат тарихын (+
байланысты заказды, бар болса) оқып, жауап жобасы мен қысқа түсінік
ұсынады — **ЕШҚАНДАЙ әрекет жасамайды** (заказ статусын өзгертпейді, ешкімге
автоматты жауап жібермейді). Модератор ұсынысты оқып, түзетіп, өз қолымен
жібереді.

**Миграция қажет емес** — бұл фича тек edge функция + Flutter UI. Тек:

1. **Функцияны деплой жасау**: Dashboard → Edge Functions → Deploy new
   function → аты `support-assistant`, коды
   `functions/support-assistant/index.ts`, **Verify JWT = ON** (тек кірген
   пайдаланушы, функция ішінде модератор екені тексеріледі).
2. Секреттер — толтыру ботымен ОРТАҚ: `GEMINI_API_KEY` (бар болса, қосымша
   секрет қажет емес).

Тексеру: модератор → Қолдау чаты → кез келген тредті ашыңыз → жоғарыдағы
🤖 басыңыз → жауап жобасы кіру өрісіне түсуі, түсінік диалогта шығуы керек.
